async function load_page() {
    ...
    firebase.auth().onAuthStateChanged(async function (user) {
        if (user) {
            ...
            const response2 = await fetch(`http://localhost:3000/usuario/1/${id}`);
            var usuario = await response2.json();
            //console.log(usuario)
            ...
            document.getElementById('txtCEP').innerHTML += usuario.cep;
            document.getElementById('txtNumero').innerHTML += usuario.numero;
            document.getElementById('txtReferencia').innerHTML += usuario.referencia;
            document.getElementById('txtComplemento').innerHTML += usuario.complemento;
